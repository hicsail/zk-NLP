...
def is_in_append_states_todelete(initial_state):
    res="("
    for val in append_states:
...
def run_dfa(dfa, text_input):
    def next_state_fun(string, initial_state):
        ...
        '''
        if is_in_append_states_todelete(curr_state)or (val_of(curr_state) == appendedAll_state):
